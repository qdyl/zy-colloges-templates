			...
			this.J(window).load(function() {
				...
				_this.J.each($images, function(index, val) {
					if(_this.J(val).width()>_this.O['view']['minSize']&&_this.J(val).height()>_this.O['view']['minSize']){
						...
						_this.J(val).addClass('images-view').wrap('<em class="view-box"><i class="view-box-inner"></i></em>')
						.parent().append('<a href="javascript:;" class="open-view" data-view-index="'+total+'"><i class="view-icon"></i></a>');
						_this.J('#view-images-items').append('<li style="background-images:url('+url+')"></li>');
					}
				});
				uWidth = total*_this.client().width;
				_this.J('#view-images-items').width(uWidth).children().css({"width":_this.client().width,"height":_this.client().height});

				function aniView(){
					_this.J('#view-images-items').stop(true,false).animate({"left":-index*_this.client().width},300, function(){
						current = index + 1;
						_this.J("#view-current").html(current);
						_this.J("#view-original-images").attr("href",imgs[index].url);
					});
				}
				...
				_this.J(".open-view").on("click",function(event){
					...
					_this.J('#view-images-items').css({"left":-index*_this.client().width});
					viewImageInfo();
				...
				_this.J(window).resize(function(){
					...
					uWidth = total*_this.client().width;
					_this.J('#view-images-items').css({"width":uWidth,"left":-index*_this.client().width}).children().css({"width":_this.client().width,"height":_this.client().height});
				});
